...
void DataTransformer<Dtype>::TransformImgAndSeg(
 ...
 // CHECK(param_.has_display() || param_.has_contrast_adjustment() || param_.has_smooth_filtering())
 //   << "Must specify display and contrast adjustment and smooth filtering.";
  ...
  bool apply_flipping = (flipping_mode != 2); 
  if (apply_flipping) {
          cv::flip(cv_cropped_img, cv_cropped_img, flipping_mode);
          cv::flip(cv_cropped_seg, cv_cropped_seg, flipping_mode);
          if (display && phase_ == TRAIN)
                  cv::imshow("Flipping and Reflection", cv_cropped_img);
  }
  ...
  if ( smooth_filtering && apply_smooth ) {
        ...
        case 0:
           //cv::Smooth(cv_img, cv_img, smooth_type, smooth_param1);
           cv::GaussianBlur(cv_cropped_img, cv_cropped_img, cv::Size(smooth_param1,smooth_param1),0);
           break;
        case 1:
           cv::blur(cv_cropped_img, cv_cropped_img, cv::Size(smooth_param1,smooth_param1));
           break;
        case 2:
           cv::medianBlur(cv_cropped_img, cv_cropped_img, smooth_param1);
           break;
        case 3:
           cv::boxFilter(cv_cropped_img, cv_cropped_img, -1, cv::Size(smooth_param1*2,smooth_param1*2));
           break;
        }
        if (display && phase_ == TRAIN)
      cv::imshow("Smooth Filtering", cv_cropped_img);
  }
  ...
  if ( contrast_adjustment && apply_contrast ) {
    ...
    cv_cropped_img.convertTo(cv_cropped_img, -1 , alpha, beta);
        if (display && phase_ == TRAIN)
                cv::imshow("Contrast Adjustment", cv_cropped_img);
  }
  ...
  cv::Mat resized_img = cv::Mat(dsize,CV_32S);
  cv::resize(cv_cropped_img, resized_img, dsize);

  cv::Mat resized_seg = cv::Mat(dsize,CV_32S);
  cv::resize(cv_cropped_seg, resized_seg, dsize);

  cv::Point2f pt(resized_img.cols/2., resized_img.rows/2.);
  rotation_degree = apply_rotation*rotation_angle_interval;
  cv::Mat r = getRotationMatrix2D(pt, rotation_degree, 1.0);
  warpAffine(resized_img, dst_img, r, cv::Size(resized_img.cols, resized_img.rows));
  warpAffine(resized_seg, dst_seg, r, cv::Size(resized_img.cols, resized_img.rows));

  cv::Rect myROI(resized_img.cols/6, resized_img.rows/6, cv_cropped_img.cols, cv_cropped_img.rows);
  cv::Mat image_after_rotate = dst_img(myROI);
  cv::Mat label_after_rotate = dst_seg(myROI);

  image_after_rotate.copyTo(cv_cropped_img);
  label_after_rotate.copyTo(cv_cropped_seg);
  }
